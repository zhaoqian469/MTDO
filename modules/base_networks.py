...
class MSD(nn.Module):  # Ours
    ...
    def forward(self, neibor_fea, target_fea):
        # 得到第一个偏移量
        offset = torch.cat([neibor_fea, target_fea], dim=1)  # 通道连接 (4,128,64,64)
        offset1 = self.lrelu(self.offset_conv1(offset))   #卷积、激活 (4,64,64,64)
        # 得到第二个偏移量
        mul_scale_fea1 = self.lrelu(self.mul_scale1(offset1))
        mul_scale_fea2 = self.lrelu(self.mul_scale2(offset1))
        mul_scale_fea3 = self.lrelu(self.mul_scale3(offset1))  #多个尺度的卷积、激活
...
class TA_Fusion(nn.Module):
    ...
    def forward(self, group_fea):
        ...
        emb = self.tAtt_1(group_fea.contiguous().view(-1, C, H, W)).contiguous().view(B, N, -1, H, W)
        ...
        correlation_pro = correlation_pro.unsqueeze(2).repeat(1, 1, C, 1, 1).contiguous().view(B, -1, H, W)
        modulated_fea = group_fea.contiguous().view(B, -1, H, W) * correlation_pro
...
class SELayer(torch.nn.Module):   # Channel attention
    ...
    def forward(self, x):
        b, c, _, _ = x.size()
        y = self.avg_pool(x).contiguous().view(b, c)
        y = self.fc(y).contiguous().view(b, c, 1, 1)
...
class ResnetBlock(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.activation is not None:   # 比论文里多了一个激活
            out = self.act(out)
            
        return out   # 如果个多个序列resblock，out是下个块的输入x
